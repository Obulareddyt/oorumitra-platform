-- OoruMitra — Initial Schema (MySQL)
...
CREATE TABLE vehicle_work_listings (
    ...
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    updated_at DATETIME(6),
    CONSTRAINT fk_vw_user FOREIGN KEY (user_id) REFERENCES users(id)
);
...
CREATE TABLE chat_conversations (
    ...
    listing_id BIGINT,
    last_message TEXT,
    last_message_at DATETIME(6) DEFAULT CURRENT_TIMESTAMP(6),
...
CREATE TABLE rating_tags (
    ...
    CONSTRAINT fk_rtag_rating FOREIGN KEY (rating_id) REFERENCES ratings(id) ON DELETE CASCADE
);
