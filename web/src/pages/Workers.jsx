...
export default function Workers() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Worker Services 👷</h1>
        <p className="text-gray-500 text-sm mt-0.5">Find skilled labour for farming, construction & more</p>
      </div>

      {/* Filter */}
      <div className="flex gap-2 flex-wrap mb-6">
        {WORK_TYPES.slice(0, 8).map((t) => (
          <button
            key={t}
            onClick={() => { setWorkType(t); setPage(0) }}
          ...
          >
            {t === 'ALL' ? 'All Types' : workTypeIcon[t] + ' ' + workTypeLabel(t)}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <EmptyState icon="👷" title="No workers found" description="No listings match the selected filter." />
      ) : (
        <>
