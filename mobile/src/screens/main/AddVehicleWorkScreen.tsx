...
const TYPES: VehicleWorkType[] = ['PLOUGHING', 'THRESHING', 'ROTAVATOR', 'SPRAYING', 'HARVESTING', 'LEVELLING', 'OTHER'];

const AddVehicleWorkScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [workType, setWorkType] = useState<VehicleWorkType>('PLOUGHING');
  ...
  const handleSubmit = async () => {
    if (!title.trim() || !village.trim() || !pricePerAcre) {
      Alert.alert('Error', 'Please fill all required fields');
      return;
    }
    setIsLoading(true);
    try {
      await vehicleWorkService.create({title, description, vehicleWorkType: workType, village, pricePerAcre: Number(pricePerAcre), isAvailable});
  ...
  return (
    <KeyboardAvoidingView style={{flex: 1}} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <TextInput style={styles.input} value={title} onChangeText={setTitle} placeholder="e.g. JD Tractor for Ploughing" placeholderTextColor={Colors.textHint} />
